...
#pretrained_model_name = "nvidia/segformer-b5-finetuned-cityscapes-1024-1024"#"nvidia/mit-b5"#add_segformer

student_model = Mask2FormerForUniversalSegmentation.from_pretrained(
    pretrained_model_name,
).to(device)

teacher_model = Mask2FormerForUniversalSegmentation.from_pretrained(
    pretrained_model_name,
).to(device)
...
for epoch in range(args.epochs):  # 에폭
    ...
    if epoch < warmup_epoch:
        ...
        for i, data in enumerate(zip(tqdm(dataloader), target_loader)):
            ...
            with torch.no_grad():
                ...
                pred_t=pred_t['masks_queries_logits']#add_mask2former
            ...
            pred_all=pred_all['masks_queries_logits']#add_segformer
    ...
    for image, mask in val_dataloader:
        ...
        with torch.no_grad():
            ...
            s_pred=s_pred['masks_queries_logits'] #add_segformer
            t_pred=t_pred['masks_queries_logits'] #add_segformer
    ...
    if args.debug_mode == False: # for logging wandb panels
        with torch.no_grad():
            ...
            test_out=test_out['masks_queries_logits'] #add_segformer
            target_out=target_out['masks_queries_logits'] #add_segformer
            test_t_out=test_t_out['masks_queries_logits'] #add_segformer
            s_pred = nnf.interpolate(s_pred, size=(args.resize, args.resize), mode='bicubic', align_corners = False)
            t_pred = nnf.interpolate(t_pred, size=(args.resize, args.resize), mode='bicubic', align_corners = False)
